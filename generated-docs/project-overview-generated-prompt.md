...
- 注意
    - 小さく始める
        - 勝利判定が難しいので注意、専用ソースに切り分ける想定
- 最初は1つのmission
    - 右上にある1文字を消せ、など画面のどこか
    - 1文字を書け
    - wordを別wordに置き換えろ
    - 縦に4つ並んだ同じ数値を書き換えろ
    - 縦に4つ並んだ同じwordを書き換えろ
    - 狙い、1つのことを「まったくできない」から「ある程度できる」まで習熟する
- 次は2つのmissionがランダムに登場する
    - 狙い、「状況に応じて脳を切りかえる、使い分ける」が身につく
- で2,3,4,...n,と増えていく
- 勝利判定と、敗北かつやりなおし判定が鍵
    - まずはシンプル実装で
...
---
Generated at: 2025-12-29 07:03:53 JST
